    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.sessionManagement(management -> management.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(Authorize -> Authorize
                        ...
                        // 2. Admin endpoints
                        .requestMatchers("/api/v1/admin/**").hasAnyRole("SHOP_OWNER", "ADMIN") //if any api end point starting from api/admin - only the uses having these roles can access admin api endpoints, other users cannot access these
